...
import com.sport.util.DataHandlerTypes;
...
public class AdminServlet extends HttpServlet {
    ...
    public void init() throws ServletException {
        ...
        getServletContext().setAttribute(APPLICATION_DATA_MODELL,FactoryProducer.createFactory(DataHandlerTypes.STATIC_MEMORY));
    }
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        while (params.hasMoreElements()) {
            ...
            LOGG.log(Level.INFO, "{0}:{1}", new Object[]{key, request.getParameter(key)});
        }
        ...
        switch (request.getPathInfo()) {
            case Constants.SPORTTYPE_COMMAND_ADD:
                FactoryProducer.getInstance().getSportTypeDAO().registerIfNotExist(
            ...
            case Constants.SEASON_COMMAND_ADD:
                FactoryProducer.getInstance().getSeasonDAO().registerIfNotExist(
                        request.getParameter(Constants.FORM_PARAM_NAME),
                        request.getParameter(Constants.FORM_PARAM_DESCRIPTION)
                );
            ...
            case Constants.SERIES_COMMAND_ADD:
                ...
                for(String s:sports){
                    selectedSports.add(FactoryProducer.getInstance().getSportTypeDAO().getByName(s));
                }
                FactoryProducer.getInstance().getSeriesDAO().createIfNotExist(
                        request.getParameter(Constants.FORM_PARAM_NAME),
                        request.getParameter(Constants.FORM_PARAM_DESCRIPTION),
                        selectedSports
                );
            ...
            case Constants.CHAMPIONSHIP_COMMAND_ADD:
                sportType = FactoryProducer.getInstance().getSportTypeDAO().getByName(request.getParameter(Constants.FORM_PARAM_SPORTTYPE));
                season = FactoryProducer.getInstance().getSeasonDAO().getByName(request.getParameter(Constants.FORM_PARAM_SEASON));
                series = FactoryProducer.getInstance().getSeriesDAO().getByName(request.getParameter(Constants.FORM_PARAM_SERIES));
                FactoryProducer.getInstance().getChampionshipDAO().createIfNotExist(
                        request.getParameter(Constants.FORM_PARAM_NAME),
                        request.getParameter(Constants.FORM_PARAM_DESCRIPTION),
                        sportType,
                        season,
            ...
            case Constants.COMPETITION_COMMAND_ADD:
                ...
                season = FactoryProducer.getInstance().getSeasonDAO().getByName(request.getParameter(Constants.FORM_PARAM_SEASON));
                series = FactoryProducer.getInstance().getSeriesDAO().getByName(request.getParameter(Constants.FORM_PARAM_SERIES));
                championship = FactoryProducer.getInstance().getChampionshipDAO().getByName(request.getParameter(Constants.FORM_PARAM_CHAMPIONSHIP));
                FactoryProducer.getInstance().getCompetitionDAO().createIfNotExist(
                        request.getParameter(Constants.FORM_PARAM_NAME),
                        request.getParameter(Constants.FORM_PARAM_DESCRIPTION),
                        championship
                );
            ...
            case Constants.RACE_COMMAND_ADD:
                ...
                if(selectedCompetiton == null || selectedCompetiton.isEmpty()){                
                    ...
                    competition = FactoryProducer.getInstance().getCompetitionDAO().getByName(championship, request.getParameter(Constants.FORM_PARAM_COMPETITION));
                    FactoryProducer.getInstance().getRaceDAO().createIfNotExist(request.getParameter(Constants.FORM_PARAM_NAME), request.getParameter(Constants.FORM_PARAM_DESCRIPTION), competition);
            ...
            case Constants.RACE_COMMAND_LIST:
                ...
                if (sportType != null && season != null && championship != null && competition != null) {
                    ...
                    request.setAttribute("races", FactoryProducer.getInstance().getRaceDAO().getRaces(championship, competition));
                }
                request.setAttribute("page", Constants.RACE_COMMAND_LIST);
                break;
